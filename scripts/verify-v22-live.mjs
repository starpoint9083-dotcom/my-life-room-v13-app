...
const specs=[
 ...
 ["service-worker-v23-shell","/sw.js",b=>b.includes("my-life-room-v23-shell")&&b.includes('const BUILD="visualfix1"')&&b.includes('const PATCH="cleanup2"')&&b.includes("/core-runtime-v21.js")&&b.includes("/continuity-runtime-v22.js")&&b.includes("/cinema-background-v23.js")]
];
async function check(name,path,predicate){
 try{
  const u=new URL(base+path);u.searchParams.set("v22_verify",Date.now().toString());
  const r=await fetch(u,{cache:"no-store",headers:{"cache-control":"no-cache"}}),body=await r.text();
  return {name,status:r.status,ok:r.ok&&predicate(body),preview:body.slice(0,160)};
 }catch(e){return {name,status:0,ok:false,error:e.message}}
}
let last=[];
for(let attempt=1;attempt<=12;attempt++){
 ...
 if(last.every(x=>x.ok)){console.log("DEPLOYMENT VERIFIED P2 V22 CONTINUITY + VISUAL CLEANUP2 UNDER V23 SHELL");process.exit(0)}
