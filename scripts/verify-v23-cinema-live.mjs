...
for(let attempt=1;attempt<=12;attempt++){
 try{
  ...
  const quality=await fetch(base+"/api/cinema/batch/quality-public",{cache:"no-store"}),qualityBody=await quality.text();let qualityJson={};try{qualityJson=JSON.parse(qualityBody)}catch{}
  ...
  const qualitySafe=quality.ok&&qualityJson.ok===true&&Number(qualityJson.total)===9&&typeof qualityJson.technicalPass==="boolean"&&Number.isFinite(Number(qualityJson.technicalScore))&&qualityJson.visualReview==="pending"&&qualityJson.paidAiTriggered===false&&Array.isArray(qualityJson.clips)&&!qualityBody.includes("deviceId")&&!qualityBody.includes("device_id")&&!qualityBody.includes("r2_key")&&!qualityBody.includes("master.jpg");
  ...
  console.log(`${ok?"PASS":"WAIT"} V23 cinema background live attempt ${attempt}/12 health=${health.status} runtime=${runtime.r.status} background=${background.r.status} route=${route.status} batch=${batch.status} latest=${latest.status} quality=${quality.status}`);
  if(ok){passed=true;break}last=JSON.stringify({h,runtime:runtime.body.slice(0,120),background:background.body.slice(0,120),sw:sw.body.slice(0,160),route:routeBody.slice(0,120),batch:batchBody.slice(0,120),publicBad:publicBadBody.slice(0,120),latest:latestBody.slice(0,220),quality:qualityBody.slice(0,320)})
...
console.log("DEPLOYMENT VERIFIED P2 V23 CINEMA BACKGROUND + PUBLIC PROGRESS + ZERO-COST TECHNICAL QUALITY MANIFEST: 9 CLIPS -> 27 FLOWS, NO PAID GENERATION OR VISUAL AI TRIGGERED");
