...
for(const s of ["v29-scene-manager","sceneSetup","장면 넣기","미리보기","/api/scene-library/upload","/api/scene-library/delete","refreshAssets"])if(!manager.includes(s))errors.push(`manager missing ${s}`);
...
for(const text of [api,manager])if(text.includes("/api/motion/generate")||text.includes("confirm_cost:true")||text.includes("minimax/hailuo"))errors.push("V29 manager must not contain paid video generation path");
if(errors.length){console.error(errors.join("\n"));process.exit(1)}
console.log("SCENE MANAGER V29 SELFCHECK PASSED: private R2 upload + D1 registry + mobile replace/preview/delete + no paid generation");
