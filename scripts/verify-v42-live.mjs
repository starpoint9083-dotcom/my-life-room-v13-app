...
const retired=["/frame-motion-v27.js","/scene-runtime-v28.js","/scene-manager-v29.js","/scene-program-v30.js","/master-motion-v32.js","/master-motion-v33.js","/master-motion-v34.js","/master-motion-v35.js","/master-motion-v36.js","/motion-qc-v37.js","/motion-auto-audit-v38.js"];
let last="";for(let attempt=1;attempt<=12;attempt++){try{
 const [home,js,manifestText,css,statusText,oldJs,oldCss,reportText]=await Promise.all([must(`/?v42_master_only=${Date.now()}`),must("/master-frames-v42.js?v=42"),must("/master-frames-v42-manifest.json?v=42"),must("/master-scenes-v40.css?v=42-master-only"),must("/api/master-scenes/status"),must("/master-motion-v41.js?v=retired"),must("/master-motion-v41.css?v=retired"),must("/master-frames-v42/generation-report.json?v=42")]);
 const manifest=JSON.parse(manifestText),status=JSON.parse(statusText),report=JSON.parse(reportText);
 if(!home.includes("/master-frames-v42.js?v=42")||!home.includes("/master-scenes-v40.css?v=42-master-only"))throw new Error("V42 master-only injection missing");
 for(const file of retired)if(home.includes(file))throw new Error(`legacy motion still injected: ${file}`);
 if(home.includes("/master-motion-v41.js")||home.includes("/master-motion-v41.css"))throw new Error("V41 fake motion still injected");
 if(!css.includes("master40-shell")||!css.includes("Old room/avatar/pet/fake-motion layers must never reappear"))throw new Error("master-only CSS markers missing");
 ...
 console.log(`PASS V42 master-only live attempt ${attempt}/12 masters=8/8 routes=9 frames=64/64 bytes=${bytes} legacy-motion=purged master-only=ready`);process.exit(0)
 }catch(e){last=e?.message||String(e);console.log(`WAIT V42 master-only live attempt ${attempt}/12 ${last}`);if(attempt<12)await new Promise(r=>setTimeout(r,5000))}}
console.error("P2 V42 master-only live verification failed",last);process.exit(1);
