...
const errors=[];
for(const s of ["borderStats","alphaAudit","unsafe cutout","lastAvatarCutout","lastPetCutout","visual17Cutout=cutout","visual17GetMasterCutout"])if(!visual.includes(s))errors.push(`visual missing ${s}`);
...
if(!sw.includes('const BUILD="visualfix1"'))errors.push("service worker hotfix build marker missing");
if(errors.length){console.error(errors.join("\n"));process.exit(1)}
console.log("VISUAL HOTFIX SELFCHECK PASSED");
