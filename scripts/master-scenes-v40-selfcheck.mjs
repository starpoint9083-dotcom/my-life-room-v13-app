...
if(!errors.length){
 ...
 for(const s of ['v40-eight-master-assets','showMaster','showForSlot','masterMotionV36','master_04_return_home','master_08_night_rest'])if(!runtime.includes(s))errors.push(`runtime missing ${s}`);
 ...
 for(const text of [runtime,api,worker])if(/hailuo|minimax|motion\/generate|confirm_cost\s*:\s*true/i.test(text))errors.push('paid generation path found in V40 stack');
}
if(errors.length){console.error(errors.join('\n'));process.exit(1)}
console.log('P2 V40 MASTER SCENES SELFCHECK PASSED: 8 canonical originals + R2 storage + actual room display + V34/50-slot bridge + free-only');
