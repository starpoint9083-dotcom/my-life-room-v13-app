...
function paintQc(text,{done=false,error=false}={}){ensureQcUi();const p=qcProgress();if(p)p.textContent=text;const b=qcBtn();if(b){b.disabled=!error&&!done;b.textContent=done?"시각검사 완료":error?"시각검사 다시 시작":"시각검사 진행 중 · 화면 닫아도 됩니다"}}
...
async function refreshQcPublic(){ensureQcUi();try{const r=await fetch("/api/cinema/batch/visual-qc/latest-public",{cache:"no-store"}),d=await r.json();if(!r.ok||!d.ok)return null;const p=qcProgress(),b=qcBtn();if(d.status==="complete"){if(p)p.textContent=`완료 · ${d.ready}/9 · 시각점수 ${d.visualScore}점 · 재검토 ${d.candidates?.length||0}개`;if(b){b.disabled=true;b.textContent="시각검사 완료"}}else if(d.status==="running"||d.status==="queued"){if(p)p.textContent=`시각검사 중 · ${d.ready||0}/9 · 현재 ${d.visualScore||0}점`;if(b){b.disabled=true;b.textContent="시각검사 진행 중 · 화면 닫아도 됩니다"}}return d}catch{return null}}
async function pollQcOnce(){const id=qcJobId();if(!id){await refreshQcPublic();return false}let job=null;try{const r=await fetch(`/api/cinema/batch/status?id=${encodeURIComponent(id)}`,{headers:auth(),cache:"no-store"});job=await r.json();if(!r.ok||!job.ok)throw new Error(job.error||`status ${r.status}`)}catch(e){paintQc("시각검사 서버 상태를 다시 확인하는 중…");return true}const q=await refreshQcPublic();if(job.status==="complete"){saveQcJob("");clearInterval(qcPolling);qcPolling=null;paintQc(`완료 · ${q?.ready||9}/9 · 시각점수 ${q?.visualScore??"-"}점 · 재검토 ${q?.candidates?.length||0}개`,{done:true});return false}if(job.status==="errored"||job.status==="terminated"){saveQcJob("");clearInterval(qcPolling);qcPolling=null;paintQc(`시각검사 중단 · ${q?.ready||0}/9까지 검사됨`,{error:true});return false}paintQc(`시각검사 중 · ${q?.ready||0}/9 · 현재 ${q?.visualScore||0}점`);return true}
function startQcPolling(){clearInterval(qcPolling);pollQcOnce();qcPolling=setInterval(pollQcOnce,7000)}
async function startVisualQc(){if(!window.confirm("완성된 9개 Cinema 기준 프레임을 Workers AI가 시각검사합니다. AI 사용 비용이 발생할 수 있습니다. 검사를 시작할까요?"))return;ensureQcUi();const b=qcBtn();if(b){b.disabled=true;b.textContent="시각검사 시작 중…"}try{const r=await fetch("/api/cinema/batch/visual-qc/start",{method:"POST",headers:auth({"content-type":"application/json"}),body:JSON.stringify({confirm_cost:true})}),d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||`visual qc ${r.status}`);saveQcJob(d.id);paintQc("유료 시각검사를 시작했습니다. 이제 화면을 닫아도 됩니다.");startQcPolling()}catch(e){paintQc(`시각검사 시작 실패 · ${e.message}`,{error:true})}}
