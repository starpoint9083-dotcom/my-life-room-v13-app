...
function avgCorner(data,w,h){
  const pts=[];const n=7;
  for(const [ox,oy] of [[0,0],[w-n,0],[0,h-n],[w-n,h-n]])for(let y=0;y<n;y++)for(let x=0;x<n;x++){const i=((oy+y)*w+(ox+x))*4;pts.push([data[i],data[i+1],data[i+2]])}
  const sum=pts.reduce((a,p)=>[a[0]+p[0],a[1]+p[1],a[2]+p[2]],[0,0,0]);return sum.map(v=>v/pts.length);
}
async function legacyConservativeCutout(src){
  ...
  const frame=ctx.getImageData(0,0,w,h),d=frame.data,bg=avgCorner(d,w,h),count=w*h,seen=new Uint8Array(count),q=new Int32Array(count);let head=0,tail=0;
  const threshold2=34*34,near=i=>{const p=i*4,dr=d[p]-bg[0],dg=d[p+1]-bg[1],db=d[p+2]-bg[2];return dr*dr+dg*dg+db*db<threshold2},push=i=>{if(i<0||i>=count||seen[i]||!near(i))return;seen[i]=1;q[tail++]=i};
  for(let x=0;x<w;x++){push(x);push((h-1)*w+x)}for(let y=0;y<h;y++){push(y*w);push(y*w+w-1)}while(head<tail){const i=q[head++],x=i%w;d[i*4+3]=0;if(x>0)push(i-1);if(x<w-1)push(i+1);if(i>=w)push(i-w);if(i<count-w)push(i+w)}
  ctx.putImageData(frame,0,0);return c.toDataURL("image/png");
}
async function cutout(src,kind="avatar"){
  if(!src)return null;
  if(typeof window.visual17Cutout==="function")return window.visual17Cutout(src,kind);
  const key=`legacy-cutout:${kind}:${src}`;if(memory.has(key))return memory.get(key);const out=await legacyConservativeCutout(src);memory.set(key,out);return out;
}
function fit(img,kind){try{if(typeof window.visual17FitActor==="function")window.visual17FitActor(img,kind)}catch{}}
...
async function enhanceRoom(){
  ...
  const url=await ensureAsset({type:"room",roomStyle:normalizeStyle(),level,time});if(img.src!==new URL(url,location.href).href){await imageLoad(url);img.src=url}room.classList.add("visual17","hasArt");
}
async function enhancePet(){
  const pet=byId("pet"),img=byId("petAsset"),room=byId("room");if(!pet||!img)return false;
...
async function applyVisual(){
  ...
  await enhanceAvatar();const key=styleKey();if(visualBusy&&key===lastSceneKey)return;lastSceneKey=key;visualBusy=true;
...
function install(){
  ...
  window.applyVisualV17=applyVisual;window.visual17RestoreMaster=restoreMasterVisual;window.visual17GetMasterCutout=()=>lastAvatarCutout;window.visual17GetPetCutout=()=>lastPetCutout;
}
