...
const MASTER_ENGINE="/master-motion-v34.js?v=34";
...
function master(){return window.masterMotionV34||window.masterMotionV33||window.masterMotionV32||null}
...
function status(){const st=effectiveState(),mm=master()?.status?.()||null;return {version:VERSION,running,lastMode,readyScenes:ready().length,mappedFallbackSlots:mm?.mappedSlots||0,currentLocation:st.currentLocation,currentPose:st.currentPose,recent:[...recent],masterMotion:mm,freeOnly:true}}
...
function loadMaster(){if(window.masterMotionV34||masterRequested||document.querySelector('script[data-master-motion-v34]'))return;masterRequested=true;const s=document.createElement("script");s.src=MASTER_ENGINE;s.async=true;s.dataset.masterMotionV34="1";document.head.appendChild(s)}
async function waitMaster(){loadMaster();for(let n=0;n<32&&!master();n++)await sleep(50);return master()}
async function ensureLocation(target){if(!target)return true;const st=effectiveState();if(st.currentLocation===target)return true;const sc=scene();try{const ok=await sc?.goTo?.(target);if(ok)return true}catch{}const mm=await waitMaster();try{if(await mm?.playTransition?.(st.currentLocation,target))return true}catch(e){console.warn("master transition fallback",e)}return false}
async function playCandidate(item,ctx){const sc=scene();if(!item?.s)return false;const s=item.s,target=s.start?.location||"";const mm=await waitMaster();if(s.ready===true&&sc?.playScene){const st=effectiveState();if(target&&st.currentLocation!==target&&!await ensureLocation(target))return false;const ok=await sc.playScene(s.id,{fallback:false,keepStage:false});if(ok){remember(s.id);return true}}
 if(mm?.playForSlot&&mm.canPlaySlot?.(s.id)){const st=effectiveState();if(target&&st.currentLocation!==target)await ensureLocation(target);const ok=await mm.playForSlot(s.id,{slotFallback:true});if(ok){remember(s.id);return true}}
 return false}
...
async function playMode(mode,{force=false}={}){
 ...
 try{
   ...
   if(mode==="return"){
     const mm=master();if(mm?.playForMode&&my===token){try{if(await mm.playForMode("return")){remember("master_v34_return_chain");return true}}catch(e){console.warn("v34 return",e)}}
   }
   let played=0,tries=0;
   while(my===token&&played<max&&tries<max*5){tries++;const stNow=effectiveState();const ranked=candidates(ctx.groups,{...ctx,location:stNow.currentLocation||ctx.location,pose:stNow.currentPose||ctx.pose});if(!ranked.length)break;const top=ranked[0].n,choicePool=ranked.filter(x=>x.n>=top-2).slice(0,5),choice=choicePool[Math.floor(Math.random()*choicePool.length)]||ranked[0];if(await playCandidate(choice,ctx)){played++;await sleep(90)}else recent=[choice.s.id,...recent.filter(x=>x!==choice.s.id)].slice(0,RECENT_MAX)}
