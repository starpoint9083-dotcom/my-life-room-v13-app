...
const MASTER_ENGINE="/master-motion-v35.js?v=35";
...
function master(){return window.masterMotionV35||window.masterMotionV34||window.masterMotionV33||window.masterMotionV32||null}
...
function score(s,ctx){let n=0;const tags=new Set(s.tags||[]);const mm=master();if(s.start?.location===ctx.location)n+=9;if(s.start?.pose===ctx.pose)n+=5;if(ctx.preferredLocation&&s.start?.location===ctx.preferredLocation)n+=3;for(const t of ctx.tags)if(tags.has(t))n+=2;if(s.ready===true)n+=2;n+=Number(mm?.scoreSlot?.(s.id)||0);if(recent[0]===s.id)n-=24;else if(recent.includes(s.id))n-=8;if(s.type==="transition")n-=4;return n}
...
function loadMaster(){if(window.masterMotionV35||masterRequested||document.querySelector('script[data-master-motion-v35]'))return;masterRequested=true;const s=document.createElement("script");s.src=MASTER_ENGINE;s.async=true;s.dataset.masterMotionV35="1";document.head.appendChild(s)}
async function waitMaster(){loadMaster();for(let n=0;n<40&&!master();n++)await sleep(50);return master()}
async function ensureLocation(target){if(!target)return true;const st=effectiveState(),mm=await waitMaster();const normal=mm?.normalizeLocation?.(target)||target;const current=mm?.normalizeLocation?.(st.currentLocation)||st.currentLocation;if(current===normal)return true;try{if(await mm?.playTransition?.(st.currentLocation,target))return true}catch(e){console.warn("master transition fallback",e)}const sc=scene();try{const ok=await sc?.goTo?.(target);if(ok)return true}catch{}return false}
async function playCandidate(item,ctx){const sc=scene();if(!item?.s)return false;const s=item.s,target=s.start?.location||"";const mm=await waitMaster();if(s.ready===true&&sc?.playScene){const st=effectiveState();if(target&&st.currentLocation!==target&&!await ensureLocation(target))return false;const ok=await sc.playScene(s.id,{fallback:false,keepStage:false});if(ok){remember(s.id);return true}}
 if(mm?.playForSlot&&mm.canPlaySlot?.(s.id)){const st=effectiveState();const a=mm?.normalizeLocation?.(st.currentLocation)||st.currentLocation,b=mm?.normalizeLocation?.(target)||target;if(target&&a!==b)await ensureLocation(target);const ok=await mm.playForSlot(s.id,{slotFallback:true});if(ok){remember(s.id);return true}}
 return false}
...
async function playMode(mode,{force=false}={}){
 ...
 try{
   ...
   if(mode==="return"){
     const mm=master();if(mm?.playForMode&&my===token){try{if(await mm.playForMode("return")){remember("master_v35_return_chain");return true}}catch(e){console.warn("v35 return",e)}}
   }
