...
async function enableNotify(){if(!canNotify()){invoke("toast","이 브라우저는 알림을 지원하지 않아요.");return}const p=await Notification.requestPermission();invoke("toast",p==="granted"?"위험시간 알림을 켰습니다.":"알림 권한이 허용되지 않았습니다.");render()}
function maybeNotify(kind,text){if(!canNotify()||Notification.permission!=="granted")return;const key=`${todayKey()}-${hourNow()}-${kind}`;if(lastNotify===key||localStorage.getItem("p2NotifyLast")===key)return;lastNotify=key;localStorage.setItem("p2NotifyLast",key);try{new Notification("나의 방",{body:text,tag:key})}catch{}}
function render(){mount();const state=byId("p2NowState"),text=byId("p2NowText"),btns=byId("p2NowBtns");if(!state||!text||!btns)return;btns.innerHTML="";btns.className="p2nowBtns";const h=hourNow(),rs=riskState(),habitType=habitNow();
 ...
 if((habitType==="금주"||habitType==="둘 다")&&rs.alcohol){state.textContent="금주 위험시간";text.textContent="평소 흔들리기 쉬운 시간에 가까워졌어요. 방이 먼저 알아챘습니다.";btns.append(actionButton("🍷 지금 방어하기",()=>invoke("openAlcohol"),true),messageButton());if(!canNotify()||Notification.permission!=="granted")btns.append(actionButton("알림 켜기",enableNotify));maybeNotify("alcohol","평소 술 위험시간에 가까워졌어요. 방에서 먼저 한 번 막아볼까요?");return}
 if((habitType==="금연"||habitType==="둘 다")&&rs.smoking){state.textContent="금연 위험시간";text.textContent="담배 생각이 반복되던 시간에 가까워졌어요. 길게 참지 말고 짧게 넘겨봅니다.";btns.append(actionButton("🚭 3분 방어",()=>invoke("openSmoking"),true),messageButton());if(!canNotify()||Notification.permission!=="granted")btns.append(actionButton("알림 켜기",enableNotify));maybeNotify("smoking","담배 위험시간에 가까워졌어요. 3분만 먼저 넘겨보세요.");return}
